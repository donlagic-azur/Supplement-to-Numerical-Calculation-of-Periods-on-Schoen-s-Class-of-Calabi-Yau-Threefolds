...
"=======================================";
CircA := Matrix(3,3,GetMonodromy3(`-`~(2*XA,txA),PerProdA,15)):
evalf(CircA,5);
#monodromy matrix at A -- should be integral
CircA := MTM:-round(CircA);
...
"=======================================";
CircB := Matrix(3,3,GetMonodromy3(`-`~(2*XB,txB),PerProdB)):
evalf(CircB,5);
#monodromy matrix at B &#8722;&#8722; should be integral
CircB := MTM:-round(CircB);
...
#(the above matrix needs to be integral, otherwise there has been an error in the explicit numerical calculation of integrals over some complex branches)
MatC := MTM:-round(MatC);
GetMonodromy3 := proc(tx,ToBeCombined,p := 7)
...
GetMonodromy := proc(tx,ToBeCombined,p := 7)
    local b,J11,J12,J21,J22,Res:
