...
Digits := 1000;
Y := 1;
g2 := `*`(3,16*X^4-16*X^2*Y^2+Y^4);
g3 := 64*X^6-96*X^4*Y^2+30*X^2*Y^4+Y^6;
...
txA := `+`~(YA,txOFF);
MatA := Matrix(3,3,ExplEvalAndComb3(C1,C2,txA,ResDA[3],0,2,0,[1, 1])):
evalf(MatA,5);
#(the above matrix needs to be integral, otherwise there has been an error in the explicit numerical calculation of integrals over some complex branches)
MatA := MTM:-round(MatA);
...
Typesetting[delayDotProduct](Typesetting[delayDotProduct](Typesetting[delayDotProduct](Matrix(3,3,[[1, 0, 0],
    [-4, 1, 0], [16, -8, 1]]),Matrix(3,3,[[1, 0, 0], [1, -1, 0], [1, -2, 1]]),true),Matrix(3,3,[[0, 0, 1], [0, 1,
    -2], [1, -4, 4]]),true),Matrix(3,3,[[1, 2, 1], [-1, -1, 0], [1, 0, 0]]),true);
